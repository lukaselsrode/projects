...
DEFAULT_RULES_SET=[(2,8,1),(2,0.1,0.1),(2,8,1),(0.01,0.0001,0.0001),tuple([i*constants.milli for i in [60,5,5]]),(5,0.5,0.5)]
...
# AUXILARRY FUNCTIONS
ascii_color_map,color_terminate= dict(zip(['red','green','yellow'],[f'\x1b[1;3{i};40m' for i in range(1,4)])),'\x1b[0m'
...
class KeyListner(object):
    ...
    def process_key(self, key:object) -> None:
        if key in {keyboard.Key.up, keyboard.Key.down, keyboard.Key.esc}:
            self.quit,self.direction=(True,None) if key == keyboard.Key.esc else (False,str(key).lstrip('.'))
            return
        self.direction = None
...
class Settings(object):
    ...
    def increment_settings(self):
        rv = []
        for i,v in enumerate([self.nlanes,self.asset_spawn_time,self.max_n_assets,self.pot_pwrup,self.t_per_frame,self.asset_jump_time]):
            ...
            rv.append(v)
        self.nlanes,self.asset_spawn_time,self.max_n_assets,self.pot_pwrup,self.t_per_frame,self.asset_jump_time=tuple(rv)
        self.lvl+=1
...
class Road(object):
    ...
    def update_rules(self)-> None:
        if self.game_rules.lvl==0: self.game_rules.increment_settings() # init catch
        # make sure player car doesn't go back to default lane
        if 'player_car' in self.__dict__.keys(): self.player_car.game_rules=self.game_rules
        else: self.player_car=Player(self.game_rules)
        
    def update_road_geometry(self)->None:
        self.num_lanes,self.width,self.length=self.game_rules.nlanes,(self.game_rules.nlanes)*2 , self.game_rules.window_len
        self.barrier,self.lane=self.game_rules.edge_char*self.length, color_string_on_console_stdout('yellow',self.game_rules.mid_char*round(self.length/2))        
    ...
    def flip_mid_lane(self) -> None:
        self.lane=color_string_on_console_stdout('yellow',self.lane[::-1].strip(color_terminate).strip(ascii_color_map['yellow']))
    
    def try_spawn(self) -> None:
        if (round(time.time() - self.last_spawn) > self.game_rules.asset_spawn_time) and len(self.assets) < self.game_rules.max_n_assets:
    ...
    def try_destroy(self):
        ...
        list(map(lambda i: self.remove_asset(i), freeing)),list(map(lambda f : self.shift_lane_assets(self.in_lane(f.lane_pos),-measure_asset(f.ascii)[0]),freeing))
    ...
    def spawn_obstacles(self) -> None:
        self.assets+=[Approacher(True,self.game_rules) for _ in range(random.choice(range(self.width-1)))]

    def spawn_powerup(self)->None:
        if not [i for i in self.assets if i.is_obstacle == False]:
            rng=[*[[True]*round(self.game_rules.pot_pwrup*100)], *[[False]*round((1-self.game_rules.pot_pwrup)*100)]]
    ...
    def create_ascii_asset(self,asset: object)->str:
        return color_string_on_console_stdout('red' if asset.is_obstacle else 'green',asset.ascii)
    ...
    def create_asset_lane(self,assets:list[object])->str:
        ...
        for i,a in enumerate(assets):
            if i == 0:
                if self.player_car.power == 'gun' and assets[0].lane_pos == self.player_car.lane_pos:
                    lane += self.game_rules.open_char*round((len(self.barrier)-a.frame_count)-self.player_car.bullet_spacing,3) + self.create_ascii_asset(a)
                else:
                    lane += self.game_rules.open_char*round((len(self.barrier)-a.frame_count)) + self.create_ascii_asset(a)
            else:
                lane += self.game_rules.open_char*(assets[i-1].frame_count - a.frame_count) +  self.create_ascii_asset(a)
        return lane

    def create_ascii_lane(self,lane_index:int) -> str:
        lane_assets = self.in_lane(lane_index)
        if not lane_assets: return self.create_ascii_player_area(lane_index)        
        return self.create_ascii_player_area(lane_index) + self.create_asset_lane(lane_assets)
    
    def player_hit(self) -> bool:
        lane = self.create_ascii_lane(self.player_car.lane_pos)
        frames_ahead_of_contact_point = lane.split(self.player_car.ascii)[1].split(' ') if not self.player_car.power == 'gun' else lane.split(DEFAULT_BULLET)[1].split(' ')
    ...
    def adjust_assests_after_player_move(self,previous_lane:int)->None:
        ...
        if not (initial_lane or final_lane): return
        if initial_lane: self.shift_lane_assets(initial_lane,-self.player_car.sx)
        if final_lane: self.shift_lane_assets(final_lane,self.player_car.sx)
                    
    def move_lane(self,Keys:object)->None:
        prev=self.player_car.lane_pos
        self.player_car.move_lane(Keys.direction),self.adjust_assests_after_player_move(prev)        
    ...
    def process_collision(self) -> None:
        ...
        if collides_with.is_obstacle:
            print("GAME OVER"),exit() if not powerup else self.player_car.loose_powers(),self.remove_asset(collides_with)
            return
        self.player_car.gain_powers(collides_with) if not powerup else self.remove_asset(collides_with)
...
class Player(object):
    ...
    def move_lane(self,direction:str)-> None:
        ...
        self.lane_pos=self.lane_pos-1 if 'u' in set(direction) else self.lane_pos+1
        self.lane_pos = lf if self.lane_pos < 0 else self.lane_pos and 0 if self.lane_pos > lf else self.lane_pos
    
    def update_ascii(self)-> None:
        if not self.power:
            self.ascii=DEFAULT_PLAYER_CAR
        elif 's' in set(self.power):
            self.ascii=DEFAULT_PLAYER_CAR if 'm' in set(self.ascii) else color_string_on_console_stdout('green',DEFAULT_PLAYER_CAR)
        else:
            if DEFAULT_BULLET not in set(self.ascii):
                self.ascii = DEFAULT_PLAYER_CAR+DEFAULT_BULLET
            else:
                self.bullet_spacing=(DEFAULT_BULLET_SPEED+self.bullet_spacing) % DEFAULT_BULLET_FRAME_CYCLE
                self.ascii = DEFAULT_PLAYER_CAR + self.game_rules.open_char*(self.bullet_spacing % self.game_rules.window_len) + DEFAULT_BULLET
...
class Approacher(object):
    ...
    def advance(self,Road: object)->None:
        if round(time.time()-self.last_jumped) > Road.game_rules.asset_jump_time:
            prev_lane=[i[1] for i in Road.get_asset_positions() if i[0]==self.a_id][0]
    ...
    def adjust_after_jump(self,prev:list[tuple],Road: object):
        li_assets,lf_assets=Road.in_lane(prev),list(filter(lambda x: x.a_id != self.a_id ,Road.in_lane(self.lane_pos)))
        if not li_assets and not lf_assets: return
        if li_assets: Road.shift_lane_assets(li_assets,-self.sx)
        if lf_assets: Road.shift_lane_assets(lf_assets,self.sx)
...
class Game(object):
    ...
    def process_inputs(self)->None:
        if self.Keys.direction or self.Keys.quit:
            if self.Keys.quit: print('***EXITING GAME***'),exit()
            else: self.Road.move_lane(self.Keys)
    ...
    def render(self,t,t_start) -> None:
        run('clear')
        buff,mid = '\n'*2,'\t'*3
        print('ASCII DRIVER',f"{buff*2}{mid}SCORE: {self.calc_score(t,t_start,self.Road.num_lanes)}",f'LEVEL:{self.Road.game_rules.lvl}',buff,self.Road,buff,'MOVE WITH ARROW-KEYS : [↑] & [↓]','QUIT:[ESC]',sep='\n')
